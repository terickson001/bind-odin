...
#define GB_FILE_CLOSE_PROC(name)    void        name(gbFileDescriptor fd)
  ...
  GB_DEF b32        gb_file_is_dir         (char const *filename);
...
#undef GB__SET_STD_FILE
  ...
  b32 gb_file_is_dir(char const *name) {
    u32 file_attributes = GetFileAttributes(name);
    return (file_attributes != INVALID_FILE_ATTRIBUTES &&
            file_attributes &  FILE_ATTRIBUTE_DIRECTORY);
  }

  b32 gb_dir_contents(const char *name, gbArray(char *) *entries, b32 recurse)
  {
    WIN32_FIND_DATAW data;
    void *handle = NULL;
    gbAllocator a = gb_heap_allocator();
    ...
    char path[2048];
    gb_snprintf(path, 2048, "%s\\*.*", name);

    wchar_t *w_path = gb__alloc_utf8_to_ucs2(a, path, NULL);

    if((handle = FindFirstFileW(w_path, &data)) == INVALID_HANDLE_VALUE)
      {
        gb_free(a, w_path);
        return false;
      }
    ...
    char *c_name;
    do {
      c_name = (char *)gb_ucs2_to_utf8_buf(data.cFileName);
      if (gb_strcmp(c_name, ".") != 0
          && gb_strcmp(c_name, "..") != 0)
        {
          gb_snprintf(path, 2048, "%s\\%s", name, c_name);
          w_path = gb__alloc_utf8_to_ucs2(a, path, NULL);
  ...
  b32 gb_dir_create(const char *name)
  {
    gbAllocator a = gb_heap_allocator();
    wchar_t *w_name = gb__alloc_utf8_to_ucs2(a, name, NULL);
    if (CreateDirectoryW(w_name, NULL) || GetLastError() == ERROR_ALREADY_EXISTS)
      {
        gb_free(a, w_name);
        return true;
      }
...
#undef GB__SET_STD_FILE
  ...
  gb_inline b32 gb_file_is_dir(char const *name) {
    struct stat st;
    if (stat(name, &st) == 0) return S_ISDIR(st.st_mode);
    return false;
  }

  b32 gb_dir_contents(char const *name, gbArray(char *) *entries, b32 recurse) {
    DIR *d;
    struct dirent *entry;
    char path[2048];
    ...
    while ((entry = readdir(d)) != 0) {
      gb_snprintf(path, 2048, "%s/%s", name, entry->d_name);
      if (gb_strcmp(entry->d_name, ".") == 0 || gb_strcmp(entry->d_name, "..") == 0)
        continue;
  ...
  b32 gb_dir_create(char const *name)
  {
