...
void read_wrap_config(Reader *r)
{
    while (!is_header(r) && r->data[0] != 0)
    {
        String label = read_ident(r);
        ...
        if (cstring_cmp(label, "package") == 0)
            ...
            r->conf->wrap_conf.bind_package_name = read_ident(r);
        else if (cstring_cmp(label, "prefix") == 0)
            r->conf->wrap_conf.type_prefix = r->conf->wrap_conf.var_prefix
            = r->conf->wrap_conf.proc_prefix = r->conf->wrap_conf.const_prefix = read_path(r);
        else if (cstring_cmp(label, "type-prefix") == 0)
            r->conf->wrap_conf.type_prefix = read_path(r);
        else if (cstring_cmp(label, "var-prefix") == 0)
            r->conf->wrap_conf.var_prefix = read_path(r);
        else if (cstring_cmp(label, "proc-prefix") == 0)
            r->conf->wrap_conf.proc_prefix = read_path(r);
        else if (cstring_cmp(label, "const-prefix") == 0)
            r->conf->wrap_conf.const_prefix = read_path(r);
        else if (cstring_cmp(label, "type-case") == 0)
            r->conf->wrap_conf.type_case = read_case(r);
        else if (cstring_cmp(label, "var-case") == 0)
            r->conf->wrap_conf.var_case = read_case(r);
        else if (cstring_cmp(label, "proc-case") == 0)
            r->conf->wrap_conf.proc_case = read_case(r);
        else if (cstring_cmp(label, "const-case") == 0)
            r->conf->wrap_conf.const_case = read_case(r);
        else
            reader_error(r, "Invalid label \"%.*s\" in ::/wrap\n", LIT(label));
        consume_whitespace(r, true);
        consume_whitespace(r, true);
    }
}
void read_config(Reader *r)
{
    while (r->data[0] != 0)
    {
        ...
        if (cstring_cmp(header, "general") == 0)
            ...
            read_bind_config(r);
        else if (cstring_cmp(header, "wrap") == 0)
            read_wrap_config(r);
        else
