...
#define require(k, p) _require(k, p, __FUNCTION__, __LINE__)
Token _require(TokenKind k, Parser *p, const char *caller, int from_line)
{
    if (p->curr->kind == k)
        return advance(p);
...
#define expect(k, p, tok) _expect(k, p, tok, __FUNCTION__, __LINE__)
b32 _expect(TokenKind k, Parser *p, Token *tok, const char *caller, int from_line)
{
    if (p->no_backtrack)
    {
